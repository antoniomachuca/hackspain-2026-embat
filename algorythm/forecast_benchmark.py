...
Run: python -m algorythm.forecast_benchmark --dataset data
...
from algorythm.forecast_context import load_context
from algorythm.forecast_data import (FEATURE_NAMES, INTERNAL_FEATURES, Samples, feature_panel,
                                    make_samples, partition_samples, split_groups)
from algorythm.forecast_models import CANDIDATES, Forecaster
from algorythm.forecast_stress import PROFILES, generate_stress, stress_score_report, synthetic_erp
...
def load_protocol():
    return json.loads((HERE/'forecast_protocol.json').read_text())
...
def run(dataset, output, context_path=None, allow_assumed=False, stress_n=None):
    ...
    sources = {p.name: sha256(p) for p in HERE.glob('forecast_*.py')}
    sources.update({k: sha256(HERE/k) for k in ('score_engine.py', 'score_data.py', 'score_states.py', 'score_monitor.py', 'forecast_protocol.json', 'forecast_policy_rates.json', 'behavior_benchmark.py')})
...
def main():
    ...
    parser.add_argument('--output', type=Path, default=HERE/'forecast_results')
