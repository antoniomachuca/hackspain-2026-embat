...
Carga datos desde:
  ...
  - dataset/groups.csv
  - algorithm/engine_results/scores_monthly.csv
  - algorithm/engine_results/alerts_history.json & alerts_feed.json
...
RESULTS_DIR = ROOT / "algorithm" / "engine_results"
...
def build_database(db_path: Optional[Path] = None, rebuild: bool = True, verbose: bool = True) -> Path:
    ...
    try:
        ...
        if not scores_csv.exists():
            raise FileNotFoundError(f"No se encontró {scores_csv}. Ejecute antes algorithm/calc_score.py si no está generado.")
