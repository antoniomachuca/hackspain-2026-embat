...
from algorithm.email_notifier import (
...
class TestEmailNotifier(unittest.TestCase):
    ...
    def test_force_salta_el_filtro_de_direccion(self):
        with patch('algorithm.email_notifier.smtplib.SMTP') as smtp_cls, \
             patch('algorithm.email_notifier.render_chart', return_value=None):
            smtp = MagicMock()
    ...
    def test_broadcast_envia_por_smtp(self):
        with patch('algorithm.email_notifier.smtplib.SMTP') as smtp_cls, \
             patch('algorithm.email_notifier.render_chart', return_value=PNG):
            smtp = MagicMock()
            smtp_cls.return_value.__enter__.return_value = smtp
    ...
    def test_broadcast_reporta_fallo_de_conexion(self):
        with patch('algorithm.email_notifier.smtplib.SMTP', side_effect=ConnectionRefusedError('sin Mailpit')), \
             patch('algorithm.email_notifier.render_chart', return_value=None):
            res = broadcast_alert(DETERIORO, config=CONFIG)
