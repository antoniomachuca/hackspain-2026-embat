...
from algorithm.telegram_levers import format_palancas_html, format_rankings_html, palancas_keyboard
from algorithm.telegram_notifier import build_alert_keyboard
from algorithm.score_telegram_bot import handle_callback_query
...
def test_callback_palancas():
    with patch('algorithm.score_telegram_bot.send_chat_action'), \
         patch('algorithm.score_telegram_bot.add_subscriber'), \
         patch('algorithm.score_telegram_bot.handle_palancas_query') as mock_pal, \
         patch('algorithm.score_telegram_bot.answer_callback_query'):
...
def test_callback_rank():
    with patch('algorithm.score_telegram_bot.send_chat_action'), \
         patch('algorithm.score_telegram_bot.add_subscriber'), \
         patch('algorithm.score_telegram_bot.handle_levers_rankings') as mock_rank, \
         patch('algorithm.score_telegram_bot.answer_callback_query'):
        handle_callback_query({
            'id': 'cb_r',
