...
def main():
    ...
    path = ROOT / 'algorithm' / 'formula' / 'engine_benchmark_results.tex'
    path.write_text(text, encoding='utf-8')
    print(f'Rendered {path}')
    behavior = []
    for name in ('synthetic_validation.json', 'real_holdout.json'):
        with (ROOT / 'algorithm' / 'behavior_results' / name).open(encoding='utf-8') as source:
            report = json.load(source)
        for filename, expected in report['source_sha256'].items():
            if sha256(ROOT / 'algorithm' / filename) != expected:
                raise ValueError('Trajectory results are stale; rerun their frozen evaluation.')
        behavior.append(report)
    trajectory = ROOT / 'algorithm' / 'formula' / 'trajectory_benchmark_results.tex'
