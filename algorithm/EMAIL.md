...
.venv/bin/python algorithm/email_notifier.py --test
...
.venv/bin/python algorithm/email_notifier.py --test --company COMP_0176

# Empresa que hoy no está en deterioro: --force la envía igualmente
.venv/bin/python algorithm/email_notifier.py --test --company COMP_0001 --force

# La última alerta de deterioro real del feed del monitor
.venv/bin/python algorithm/email_notifier.py --send-latest

# Destinatario explícito y sin gráfica
.venv/bin/python algorithm/email_notifier.py --test --company COMP_0176 --to hugo@ejemplo.es --no-chart
```
...
```bash
.venv/bin/python algorithm/score_monitor.py --watch --email
# ambos canales a la vez:
.venv/bin/python algorithm/score_monitor.py --watch --telegram --email
```
...
```bash
.venv/bin/python algorithm/email_notifier.py --set-recipient COMP_0010 hugo@ejemplo.es
.venv/bin/python algorithm/email_notifier.py --recipients
```

Se guarda en `algorithm/email_recipients.json`, que está en `.gitignore`.
...
```bash
.venv/bin/python -m pytest algorithm/test_email_notifier.py -q
```
