...
/**
 * Igual que en el front. Lo único que cambia: la ruta activa y si el panel
 * está desplegado entran por props, en vez de salir del router y de
 * `localStorage`, que en un render no existen.
 */
export function Shell({ children, ruta = "/", abierto = true }: {
  children: React.ReactNode; ruta?: string; abierto?: boolean;
}) {
  const path = ruta;
  const alternar = () => {};
