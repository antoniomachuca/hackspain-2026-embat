...
/**
 * `palanca` NO EXISTE EN EL FRONT. Es la única capa que el vídeo añade por su
 * cuenta: la proyección que saldría si se accionase una palanca del catálogo.
 * El cálculo es honesto —el delta sale de `simular()`, la misma función que
 * usa el simulador— pero la app todavía no enlaza las dos cosas. Si el front
 * acaba enlazándolas, esta capa se sube tal cual y deja de ser del vídeo.
 *
 * `mezcla` ∈ [0,1] lleva la proyección de la base al contrafactual; el
 * dominio vertical se calcula siempre con el destino final, para que el eje
 * no se reescale a mitad del movimiento.
 */
export function Prevision({
  ...
  // En un render no hay puntero ni estado: el hover se queda siempre apagado y
  // la ventana la elige quien monta el plano. Los manejadores se dejan puestos
  // para no tocar el marcado de la app.
  const hover: number | null = null;
  type Hover = number | null;
  const setHover = (_: Hover | ((p: Hover) => Hover)) => {};
  const setRango = (_: (typeof RANGOS)[number]) => {};
  ...
  // La palanca no reescribe el pasado: mueve el punto al que se llega. El
  // empujón entra proporcional a `mezcla` para que el cambio se vea moverse.
  const empuje = palanca ? palanca.delta * Math.max(0, Math.min(1, palanca.mezcla)) : 0;
  const central = proyeccionCentral(hoy, momentum, empuje);
  const arriba = clamp(central + amplitud);
  const abajo = clamp(central - amplitud);
  // Base sin palanca: se deja dibujada por detrás para poder comparar.
  const centralBase = proyeccionCentral(hoy, momentum);
  const arribaBase = clamp(centralBase + amplitud);
  const abajoBase = clamp(centralBase - amplitud);
  // Techo del recorrido, para que el eje no salte mientras la senda sube.
  const techo = palanca ? clamp(hoy + deriva + palanca.delta + amplitud) : arriba;
  ...
  const valores = [...datos.map((d) => d.score), ...(datosPeer?.map((p) => p.mediana) ?? []),
    arriba, abajo, techo, abajoBase];
  ...
  const sendasBase = palanca
    ? {
        Alto:  senda(hoy, arribaBase,  meses, 0.62, vol, 9001),
        Medio: senda(hoy, centralBase, meses, 0.92, vol, 9002),
        Bajo:  senda(hoy, abajoBase,   meses, 1.45, vol, 9003),
      }
    : null;
    ...
    <div className="w-full">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          {palanca && palanca.mezcla > 0.02 && (
            <p className="mt-2 inline-flex items-center gap-2 rounded-md px-2.5 py-1 text-[12px] font-medium"
              style={{ background: "rgba(128,239,162,.16)", color: "#80efa2", opacity: Math.min(1, palanca.mezcla * 2.5) }}>
              con {palanca.nombre.toLowerCase()}
              <span className="tnum">+{num(palanca.delta * Math.max(0, Math.min(1, palanca.mezcla)))} pts</span>
            </p>
          )}
        </div>
      ...
      <div className="relative -mr-6 mt-2 w-[calc(100%+1.5rem)]">
        <svg viewBox={`0 0 ${ancho} ${alto}`} className="w-full" role="img" aria-label={capas}>
          ...
          {/* De dónde venía la proyección. Sin esta referencia el movimiento
              no se lee: la línea sube, pero no se sabe desde dónde. */}
          {sendasBase && palanca && palanca.mezcla > 0.02 && (
            <g opacity={0.34 * Math.min(1, palanca.mezcla * 2.5)}>
              {(["Alto", "Medio", "Bajo"] as const).map((k) => (
                <path key={k} d={traza(sendasBase[k])} fill="none" stroke="#787d96"
                  strokeWidth="1.4" strokeDasharray="4 6" strokeLinecap="round" />
              ))}
            </g>
          )}
...
/** El escenario central: hoy más la inercia observada, más lo que aporte la
 *  palanca. Lo usa el gráfico y lo usan los rótulos del vídeo, para que un
 *  cambio en la fórmula no deje al texto diciendo otra cosa que el dibujo. */
export function proyeccionCentral(hoy: number, momentum: number, empuje = 0) {
  return clamp(hoy + momentum * 14 + empuje);
}
